...
pub extern fn terminateJ(m: &mut Master)
 ...
 #[no_mangle]
 pub extern fn serializeJ(m: &mut Master) -> *const u8
  {
      let s = m.serialize();
      s.as_ptr()
  }
...
#[no_mangle]
pub extern fn listenJ(s: &mut Subscriber, chan: *const c_char) -> *const u8
{
    let c = unsafe { CStr::from_ptr(chan) };
    let m = s.listen(c.to_str().unwrap().to_string());
    m.as_ptr()
}
